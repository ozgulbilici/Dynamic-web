...
function get_three_products() {
	...
	while ($row = fetch_array($query)) {
		...
		if ($count < 8) {
			$product = <<<DELIMETER
		    <div class="col-3">
		    	<a href="product_detail.php?id={$row['product_id']}"><img src="products/{$row['product_image']}"></a>
...
function get_products_update_page() {
	...
	while ($row = fetch_array($query)) {
		...
		<tr>
		    ...
		    <td><a href="product_detail.php?id={$row['product_id']}"><img src="products/{$row['product_image']}" width=100></td></a>
...
function get_categories() {
	...
	while ($row = fetch_array($query)) {
		...
		$category_links = <<<DELIMETER
		  ...
		  <a class= "borderexample" href="category_page.php?id={$row['category_id']}">{$row['category_title']}</a>
...
function show_categories_in_admin() {
	...
	while($row = fetch_array($category_query)) {
		...
		<tr>
		    <td>{$category_id}</td>
		    <td style="padding-left: 60px;">{$category_title}</td>
		</tr>
		DELIMETER;
...
function get_products_admin_page() {
	...
	while ($row = fetch_array($query)) {
		...
		<tr>
		    <td>{$row['product_id']}</td>
		    <td><img src="products/{$row['product_image']}" width=100></td>
...
function display_users() {
	...
	while($row = fetch_array($user_query)) {
		...
		<tr>
		    ...
		    <td><a style="color: #7f5a65; padding-left: 30px;" class="btn btn-danger" href="delete_user.php?id={$row['user_id']}">Delete User</a>
		</tr>
		DELIMETER;
